...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 30 19:17:12 2025
...
# ============================
# 🔥 Fit SARIMA Model for All Features
# ============================
...
def generate_synthetic_sarima_data(start_date="2025-01-01", years=5):
    """
    Generates SARIMA-based synthetic data for the next 5 years at hourly intervals.
    """
    # Define time range
    ...
    print("Generating synthetic data with SARIMA...")

    # Fit SARIMA for each feature and generate synthetic data
    for feature in continuous_features:
        # Fit SARIMA model to historical data
        train_data = df[["DateTime", feature]].set_index("DateTime")
        
        # Fit SARIMA model (tune the order as needed for each feature)
        model = SARIMAX(train_data[feature], order=(1, 1, 1), seasonal_order=(1, 1, 1, 24))  # Adjust `seasonal_order` if necessary
        sarima_model = model.fit(disp=False)
        
        # Forecast for next 5 years (hourly)
        forecast = sarima_model.get_forecast(steps=len(synthetic_data))
        forecast_values = forecast.predicted_mean

        # Store the synthetic data for the feature
        synthetic_features.append(forecast_values)

    # Combine all synthetic features into the synthetic_data DataFrame
    synthetic_data[continuous_features] = np.array(synthetic_features).T
...
synthetic_5_years = generate_synthetic_sarima_data()

# Display sample output
print(synthetic_5_years.head())

# Save the synthetic data
output_file = "synthetic_sarima_data_5_years.csv"
...
plt.title("🕒 SARIMA-Based Synthetic Demand Trends for 2025")
plt.xlabel("Time")
